...
class ImageTest < Test::Unit::TestCase
  def test_create_image
    ...
    image = SpiffyStoresAPI::Image.new(:product_id => 632910392)
    image.position = 1
    ...
    assert_equal 'http://cdn.spiffyserver.com/s/files/1/0006/9093/3842/products/ipod-nano.png?v=1389388540', image.src
    assert_equal 850703190, image.id
  end

  def test_get_images
    fake "products/632910392/images", :method => :get, :body => load_fixture('images')
    image = SpiffyStoresAPI::Image.find(:all, :params => {:product_id => 632910392})
    assert_equal 2, image.size
  end

  def test_get_image
    fake "products/632910392/images/850703190", :method => :get, :body => load_fixture('image')
    image = SpiffyStoresAPI::Image.find(850703190, :params => {:product_id => 632910392})
    assert_equal 850703190, image.id
  end
  ...
  def test_delete_image_without_fetching
    fake "products/632910392/images/850703190", :method => :delete, :body => "destroyed"
    image = SpiffyStoresAPI::Image.new(product_id: 632910392, id: 850703190)
    assert image.destroy
  end
end
