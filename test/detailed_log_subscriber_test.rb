...
class LogSubscriberTest < Test::Unit::TestCase
  ...
  def setup
    super
    @page = { :page => { :id => 1, :title => 'SpiffyStores API' } }.to_json
    @ua_header = "\"User-Agent\"=>\"SpiffyStoresAPI/#{SpiffyStoresAPI::VERSION} ActiveResource/#{ActiveResource::VERSION::STRING} Ruby/#{RUBY_VERSION}\""
  ...
  test "logging on #find" do
    ...
    SpiffyStoresAPI::Page.find(1)

    assert_equal 4, @logger.logged(:info).size
    assert_equal "GET https://this-is-my-test-shop.spiffystores.com:443/api/pages/1.json", @logger.logged(:info)[0]
    assert_match /\-\-\> 200/, @logger.logged(:info)[1]
    assert_equal "Headers: {\"Accept\"=>\"application/json\", #{@ua_header}}", @logger.logged(:info)[2]
    assert_match /Response:\n\{\"page\"\:\{((\"id\"\:1)|(\"title\"\:\"SpiffyStores API\")),((\"id\"\:1)|(\"title\"\:\"SpiffyStores API\"))\}\}/,  @logger.logged(:info)[3]
  ...
  test "logging on #find with an error" do
    ...
    assert_raises ActiveResource::ResourceNotFound do
      SpiffyStoresAPI::Page.find(2)
    end

    assert_equal 4, @logger.logged(:info).size
    assert_equal "GET https://this-is-my-test-shop.spiffystores.com:443/api/pages/2.json", @logger.logged(:info)[0]
