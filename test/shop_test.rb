...
class StoreTest < Test::Unit::TestCase
  ...
  def test_current_should_return_current_shop
    assert @shop.is_a?(SpiffyStoresAPI::Store)
    assert_equal "Apple Computers", @shop.name
    assert_equal "apple.spiffystores.com", @shop.spiffystores_domain
  ...
  def test_current_with_options_should_return_current_shop
    fake "shop.json?fields%5B%5D=name&fields%5B%5D=spiffystores_domain", :extension => false, :method => :get, :status => 201, :body => load_fixture('shop')

    @shop = SpiffyStoresAPI::Store.current(params: { fields: [:name, :spiffystores_domain]})
    assert @shop.is_a?(SpiffyStoresAPI::Store)
    assert_equal "Apple Computers", @shop.name
    assert_equal "apple.spiffystores.com", @shop.spiffystores_domain
  end
  ...
  def test_add_metafield
    ...
    field = @shop.add_metafield(SpiffyStoresAPI::Metafield.new(:namespace => "contact", :key => "email", :value => "123@example.com", :value_type => "string"))
